(ns filmdistops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (FilmDistAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the FilmDistGovernor (:govern)
  ...
  human operator/distribution coordinator. The approver resumes with
...
(defn build
  ...
  `filmdistops.store/Store`).
  opts:
    :advisor      -- a `filmdistops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FilmDistAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; FilmDistGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
